...
class Libris < Source
  ...
  XML_SCHEMA = nil
  LIBRIS_XSEARCH_MARC = "https://libris.kb.se/xsearch?format=marcxml&format_level=full&holdings=true&query=ONR:"
  ...
  def self.data_from_record(librisdata)
    ...
    if (record)
      ...
      year = nil
      if (marc_record['264'] && marc_record['264']['c'])
        year = marc_record['264']['c'].gsub(/[^\d]/,'').to_i
      elsif (marc_record['260'] && marc_record['260']['c'])
        year = marc_record['260']['c'].gsub(/[^\d]/,'').to_i
      end
      if year && year >= 1000 && year < (DateTime.now + 5.years).year
